...
{
    protected $signature = 'app:cleanup-old-conversions';
    protected $description = 'Delete conversion records and files older than 24 hours';
    ...
    public function handle()
    {
        ...
        foreach ([Conversion::class, mediaModel::class] as $model) {
            ...
            foreach ($oldRecords as $record) {
                if ($record->stored_path && File::exists($record->stored_path)) {
                    File::delete($record->stored_path);
                }
            ...
            $this->info("Cleaned up {$oldRecords->count()} old {$model} records.");
        }
    }
}